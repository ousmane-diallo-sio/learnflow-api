...
authController.post("/login/manager", async (req, res) => {
    ...
    try {
      ...
      if (manager?.password) {
        const passwordComparision = await comparePassword(loginInformation.password, manager.password)
...
authController.post("/login/moderator", async (req, res) => {
    ...
    try {
      ...
      if (moderator?.password) {
        const passwordComparision = await comparePassword(loginInformation.password, moderator.password)
        if (passwordComparision) {
...
authController.post("/login/student", async (req, res) => {
    ...
    try {
      ...
      if (student?.password) {
        const passwordComparision = await comparePassword(loginInformation.password, student.password)
        if (passwordComparision) {
...
authController.post("/login/teacher", async (req, res) => {
    ...
    try {
      ...
      if (teacher?.password) {
        const passwordComparision = await comparePassword(loginInformation.password, teacher.password)
        if (passwordComparision) {
